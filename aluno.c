...
//funçoes de salvamento e carregamento
...
int carregar_sistema(Serie *serie, Pilha_historico *historico, const char *nomeArquivo) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 2; j++) {
            fread(&serie[i].turmas[j], sizeof(Lista_turma), 1, arquivo);
        }
    }
    //fila de espera
    for (int i = 0; i < 3; i++) {
    ...
    int tamH;
    fread(&tamH, sizeof(int), 1, arquivo);
    
    if (tamH > 0) {
        Aluno *tempArray = (Aluno*) malloc(sizeof(Aluno) * tamH);
        for (int k = 0; k < tamH; k++) {
            fread(&tempArray[k], sizeof(Aluno), 1, arquivo);
        }
        for (int k = tamH - 1; k >= 0; k--) {
            push_pilha_historico(historico, tempArray[k]);
        }
        free(tempArray);
    }
